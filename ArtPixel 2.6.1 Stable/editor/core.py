...
class PixelArtEditor:
    ...
    def draw(self):
        ...
        try:
            ...
            if self.open_dialog_active:
                print("Отрисовка диалога открытия")  # Отладка
    ...
    def _handle_open_dialog_click(self, event):
        ...
        try:
            ...
            list_start_y = self.screen.get_height() // 2 - 100  # Примерное положение начала списка
            
            # Проверяем клик по каждому элементу списка
            for i, _ in enumerate(self.available_files):
                item_rect = pygame.Rect(
                    self.screen.get_width() // 2 - 190,  # X позиция
                    list_start_y + (i * 35),  # Y позиция (35 - высота элемента)
                    380,  # Ширина
                    30   # Высота
                )
            ...
            if self.open_dialog_ok_rect and self.open_dialog_ok_rect.collidepoint(pos):
                ...
                self._apply_open()
                return True
            elif self.open_dialog_cancel_rect and self.open_dialog_cancel_rect.collidepoint(pos):
    ...
    def get_available_files(self) -> List[str]:
        ...
        try:
            save_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "saves")
            print(f"Поиск файлов в директории: {save_dir}")  # Отладка
            
            if not os.path.exists(save_dir):
                print("Создание директории saves")  # Отладка
            ...
            for file in os.listdir(save_dir):
                if file.endswith('.png'):
                    files.append(file)
                    print(f"Найден файл: {file}")  # Отладка
            
            if not files:
                print("Файлы не найдены")  # Отладка
