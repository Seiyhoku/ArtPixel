...
class ColorManager:
    ...
    def handle_click(self, pos: Tuple[int, int]) -> bool:
        ...
        try:
            # Убираем лишний отладочный вывод
            if not hasattr(self, 'hue_bar_rect') or not hasattr(self, 'sv_square_rect'):
                return False

            # Проверка клика по компонентам
            if self.hue_bar_rect and self.hue_bar_rect.collidepoint(pos):
            ...
            logging.error(f"Ошибка в Color Picker: {str(e)}")
            return False
